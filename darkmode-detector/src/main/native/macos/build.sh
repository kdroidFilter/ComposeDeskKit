...
clang -arch arm64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_ARM64/libnucleus_darkmode.dylib" "$SRC"
strip -x "$OUT_DIR_ARM64/libnucleus_darkmode.dylib"

# Compile for x86_64
clang -arch x86_64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_X64/libnucleus_darkmode.dylib" "$SRC"
strip -x "$OUT_DIR_X64/libnucleus_darkmode.dylib"

echo "Built per-architecture dylibs:"
ls -lh "$OUT_DIR_ARM64/libnucleus_darkmode.dylib"
ls -lh "$OUT_DIR_X64/libnucleus_darkmode.dylib"
