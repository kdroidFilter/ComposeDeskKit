...
abstract class AbstractGenerateAotCacheTask : AbstractComposeDesktopTask() {
    ...
    private fun provisionJavaLauncher(appDir: File): Pair<String, Boolean> {
        val toolchainJavaExe = File(javaHome.get()).resolve("bin").let { binDir ->
        ...
        for (line in cfgFile.readLines()) {
            ...
            when {
                ...
                trimmed == "[Application]" || trimmed == "[ArgOptions]" -> {
                    inClasspath = false; inJavaOptions = false
                }
                ...
                inJavaOptions && trimmed.isNotEmpty() -> {
                    val opt = if (trimmed.startsWith("java-options=")) {
    ...
    private fun runAotCacheCreation(
        ...
        val logFile = File.createTempFile("composedeskkit-aot-", ".log")
