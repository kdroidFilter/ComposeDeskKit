@echo off
REM Compiles NucleusSslBridge.c into per-architecture DLLs (x64 + ARM64).
...
set "SCRIPT_DIR=%~dp0"
set "SRC=%SCRIPT_DIR%NucleusSslBridge.c"
...
cl /LD /O1 /GS- /nologo ^
    ...
    "%SRC%" ^
    /Fe:"%OUT_DIR_X64%\nucleus_ssl.dll" ^
    /link /NODEFAULTLIB /ENTRY:DllMain crypt32.lib kernel32.lib
if errorlevel 1 (
...
echo.
echo Built DLLs:
if exist "%OUT_DIR_X64%\nucleus_ssl.dll" echo   %OUT_DIR_X64%\nucleus_ssl.dll
if exist "%OUT_DIR_ARM64%\nucleus_ssl.dll" echo   %OUT_DIR_ARM64%\nucleus_ssl.dll
